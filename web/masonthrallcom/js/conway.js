...
function updateGrid(mainarray,cellcount,drawonce){
  ...
  $.each( mainarray, function(row,rowarr) {
    ...
    $.each( mainarray[row], function(col,data) {
      ...
      if (w.play){
        drawCell(data,row,col);
        var neighborcount = checkNeighbors(mainarray,row,col,w.cellcount);
        if (data[0] == 1) {
...
function checkNeighbors(neighbor_array,row,col,cellcount){
  if ( row > 0 && row < cellcount - 1 && col > 0 && col < cellcount - 1 ) 
  {
    var neighborcount = 0;
    neighborcount+=neighbor_array[row-1][col-1][0];
    neighborcount+=neighbor_array[row-1][col][0];
    neighborcount+=neighbor_array[row-1][col+1][0];
    neighborcount+=neighbor_array[row][col-1][0];
    neighborcount+=neighbor_array[row][col+1][0];
    neighborcount+=neighbor_array[row+1][col-1][0];
    neighborcount+=neighbor_array[row+1][col][0];
    neighborcount+=neighbor_array[row+1][col+1][0];
